...
- For stacked pull requests, first retarget each open dependent to the merged destination and verify that it remains open with its exact head unchanged; only then delete its obsolete base branch. Deleting the base can close a dependent instead of retargeting it. This ambiguity was observed on 5 September 2026 when deleting PR15's merged branch closed PR16; restoring the exact base, reopening PR16, retargeting it to `main`, and then deleting the obsolete base recovered its review state without changing its commits. Re-read checks for the new base; old-base green does not certify the new comparison.
