...
Honest Ink and the current public module display names are ratified identities under ADR-006, ADR-008, ADR-009, and ADR-011, but those decisions are not trademark clearances; counsel review remains a pre-release checkpoint. Honest Ink is the program's only name; `OpenClassroomFoundry` survives solely as an internal engine identifier and storage-path token, and the contributor attributions above are copyright records rather than product names. PECS® is a registered trademark of Pyramid Educational Consultants; this project makes no PECS alignment, equivalence, certification, or protocol claim (implementation plan, binding corrections).
