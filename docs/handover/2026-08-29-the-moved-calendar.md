...
- **A flake was sighted once and not reproduced.** `Foundry.Tests.Integration.ImageNormalizerTests.A_burned_region_destroys_the_pixels_beneath_it` failed in one full-solution run on 29 Aug 2026 and passed in isolation immediately after, then passed in two further full runs. **The name survived the output filter; the assertion message did not** — which is the very trap the [enactment handover](2026-08-29-forge-enactment-handover.md) §III warns about, sprung again in a slightly different way. It is recorded here as a *sighting*, not a diagnosis: no cause is claimed, because none was measured. If it recurs, capture the message before anything else, and remember that the last flake in this repository turned out not to be a test problem at all but a real seam (traceability finding 9).
