...
| Migration and compatibility tests | **Partial — preparation path built** | Schema 1 save/reopen, two frozen prior-package fixtures, full hostile validation, and proposed ADR-007's side-by-side compatibility host are tested. No schema-2 migration exists; ADR ratification, deployment, install, and rollback on a managed device remain human work |
| Documentation, contribution, localization workflows | **Done / human catalog pending** | README, GOVERNANCE, CONTRIBUTING, council charter, data inventory, and Gate C design exist. The deterministic 984-string catalog workflow is fail-closed; no second language is claimed until the multilingual seat reviews an actual catalog and its exact bytes are pinned |
