...
| Migration and compatibility tests | **Partial — preparation seam built** | Schema 1 save/reopen, two frozen prior-package fixtures, full hostile validation, and proposed ADR-007's side-by-side compatibility host pass 36 focused upgrade-related integration cases. The host is a source-tree evaluation tool, not part of the application publish. No schema-2 migration exists; ADR ratification, separately authorized operator packaging, deployment, install, and rollback on a managed device remain human work |
| Documentation, contribution, localization workflows | **Done / human catalog pending** | README, GOVERNANCE, CONTRIBUTING, council charter, data inventory, and Gate C design exist. The deterministic 978-string catalog workflow is fail-closed; no second language is claimed until the multilingual seat reviews an actual catalog and its exact bytes are pinned |
| SBOM, source archive, asset ledger | **Done for repository build evidence** | CI generates a pinned CycloneDX 1.7 JSON SBOM; the measured local artifact held 34 dependency components, all 34 with license evidence, and an exact restored-assets graph of 35 nodes and 51 edges with all 34 components root-reachable. The public GitHub repository is the source archive; the asset ledger is complete and CI-verified (13 pack assets + teacher-shelf provenance). A release still requires its own pre-publication and packaging rites |
...
| CI gates (R2-11) | **Done** | Warnings-as-errors, full format gate, 80% core-coverage threshold (88.6% at enablement), full-history secret scan, human-readable dependency inventory, direct/transitive vulnerability audit, pinned CycloneDX 1.7 SBOM with license evidence, and byte-identical samples; workflow actions and repository tools are exact-version pinned |
| Hostile-package suite | **Done (bounded contract)** | Entry/count/inflation ceilings; normalized closed inventory; duplicate/traversal inertness (nothing is extracted); strict JSON; semantic, snapshot, context, asset, provenance and exact-renderer correspondence; atomic staged save and upgrade cleanup. Fuzzing remains welcome, not a substitute for this contract |
