...
**Harness:** `tests/UiAutomation` — two layers. The **in-process** layer asserts what WinForms will hand to assistive technology (accessible names, roles, tab order, mnemonics, the standard-controls rule) as data. The **headed** layer launches the real app in its deterministic `--uia-harness review|capture` fixture mode and reads it back through UI Automation — the same tree NVDA and Narrator consume. Headed tests run on any interactive desktop and are **visibly skipped** (never silently passed) where none exists or `OCF_SKIP_HEADED=1`.
...
| 13 | Reopen approved → locked state audible | — | The surface has no reopen-approved path yet; extend harness when it ships |
